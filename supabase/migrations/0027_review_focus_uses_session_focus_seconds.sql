...
comment on function public.review_consistency(date, date) is
  'Per-day Effort Score (schema mtdo.review_consistency.v1, formula effort_v1) behind the Consistency heatmap, for one date range. Focus/Execute are scoped to the whole USER across every plan they have ever had (never re-filtered to the current active plan) so switching goals does not zero out history, using session_focus_seconds() (0023) for focus time -- never a re-spelled cap (fixed 0027). Progress reuses weekly_performance() per distinct ISO week the current plan touched in range, and is NULL outside that. effort_score is NULL only when the user has no active plan at all; otherwise a day with no computable component is a real 0, not missing data. Range capped at 400 days. See docs/architecture/api.md sec3k.';
