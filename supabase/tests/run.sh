...
"$PGBIN/psql" -v ON_ERROR_STOP=1 -q \
  ...
  -f "$ROOT/supabase/tests/18_review_consistency.sql" \
  -f "$ROOT/supabase/tests/19_review_time_patterns.sql" \
  -f "$ROOT/supabase/tests/20_review_momentum.sql" \
  -f "$ROOT/supabase/tests/21_study_profile.sql" \
  -f "$ROOT/supabase/tests/22_task_evidence.sql" \
  -f "$ROOT/supabase/tests/23_goal_taxonomy_evidence.sql" \
  -f "$ROOT/supabase/tests/24_checkin_visibility_evidence.sql" 2>&1 \
