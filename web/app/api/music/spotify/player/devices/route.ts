// Lists Spotify Connect devices -- the control center's Device tab. Same
// shape as the sibling routes; see token/route.ts for the pattern.
...
import { getDevices, SpotifyError } from "@/lib/music/spotify/spotify";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    console.error("[music/spotify/player/devices] couldn't mint an access token:", err);
  ...
  try {
    const devices = await getDevices(token.accessToken);
    return json({ devices });
  } catch (err) {
    ...
    console.error("[music/spotify/player/devices] couldn't list devices:", err);
    return json({ error: "Spotify didn't return your devices. Try again in a moment." }, 502);
  }
}
