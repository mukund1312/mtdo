...
export function TodayDeck({ onOpenBlock }: { onOpenBlock: (block: TodayBlock) => void }) {
  ...
  return <section className="a02-work" aria-labelledby="today-title">
    ...
    {state === "error" ? <section className="a02-product-state" role="alert"><b>Today is unavailable.</b><p>We could not load your blocks. Your route is unchanged.</p><button type="button" onClick={() => void load()}>Try again ↗</button></section> : <div className={`a02-board a02-board--today ${state === "loading" ? "is-loading" : ""}`} aria-busy={state === "loading"}>{LANES.map((lane) => {
      ...
      return <section key={lane.id} className={`a02-lane a02-today-lane a02-today-lane--${lane.id}`}><header><span>{lane.index}</span><b>{lane.label}</b><i>{state === "loading" ? "…" : laneBlocks.length}</i></header>{state === "loading" ? <LoadingBlocks /> : laneBlocks.length === 0 ? <p className="a02-lane-empty">No blocks here yet.</p> : laneBlocks.map((block) => <article className={`a02-work-unit a02-live-block ${block.claimed || block.status === "in_progress" ? "is-claimed" : ""}`} key={block.id} aria-busy={updatingId === block.id}><em>{block.status === "in_progress" ? "IN MOTION" : block.status === "done" ? "CLOSED" : "READY"}</em><button type="button" className="a02-work-open" onClick={() => onOpenBlock(block)}><strong>{block.text}</strong></button><small>{roughDuration(block.elapsed_seconds) ?? (block.notes?.trim() || "Personal route")}</small>{block.status === "in_progress" && <span className="a02-unit-pulse" aria-label="In progress" />}<label className="a02-status-control">Move to <select value={block.status} disabled={updatingId === block.id} onChange={(event) => void moveBlock(block, event.target.value as BlockStatus)}><option value="todo">Todo</option><option value="in_progress">In progress</option><option value="done">Done</option></select></label></article>)}</section>;
    })}</div>}
